...
- 선착순 쿠폰 발급에서 다음 **동시성 제어** 방식의 성능/안정성 비교
    1. **DB Lock** 기반(비관 잠금) - [설계 문서 링크]
    2. **Redis Streams**(컨슈머 그룹) - [설계 문서 링크]
    3. **Kafka + Debezium + Kafka Connect**(비동기 이벤트) - [설계 문서 링크]
---
## **사전조건**
...
- 모든 서버는 도커 컨테이너로 등록하고 각 컨테이너별 vCPU/Mem 최대 사용량 제한
  ...
  docker-compose.yml 설정 파일 → 컨테이너 설정

  docker-compose.m1.local.yml 설정 파일 → 리소스 할당 설정
